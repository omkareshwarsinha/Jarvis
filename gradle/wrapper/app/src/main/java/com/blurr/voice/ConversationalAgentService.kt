...
import com.blurr.voice.data.LocalMemoryStore
...
//        }
    ...
    private fun processUserInput(userInput: String) {
        serviceScope.launch {
            ...
            // Offline Predefined Command Matching (English/Hindi/Hinglish)
            val cmdResult = com.blurr.voice.utilities.PredefinedCommandMatcher.match(userInput, this@ConversationalAgentService)
            if (cmdResult.matched) {
                pandaStateManager.setState(PandaState.PROCESSING)
                speakAndThenListen(cmdResult.replyText)
                conversationHistory = addResponse("model", cmdResult.replyText, conversationHistory)
                trackMessage("model", cmdResult.replyText, "predefined")
                cmdResult.executeBlock?.let { block ->
                    serviceScope.launch {
                        block(this@ConversationalAgentService, com.blurr.voice.api.Finger(this@ConversationalAgentService))
                    }
                }
                return@launch
            }
...
//    }
    ...
    private fun initializeConversation() {
        ...
        val systemPrompt = """
            ...
            9. LANGUAGE CONTROL: You MUST perfectly recognize, understand, and process inputs in Hinglish (Hindi written in Roman script such as "gaana chalao", "selfie khicho", "camera kholo par photo click karo", "jarvis click photo"). Also, respond back in the same friendly Hinglish/Hindi or English dialect matching the user's current style.
            10. AUTOMATION FLOWS: If the user commands an action that can be executed via Intents followed by UI actions (like playing a song or taking a selfie/photo), return "Type": "Task" with appropriate instructions, or rely on predefined command triggers.
        ...
        conversationHistory = addResponse("user", systemPrompt, emptyList<Pair<String, List<Any>>>())
    }
    ...
    private fun parseModelResponse(response: String): ModelDecision {
        ...
        if (!trimmed.startsWith("{")) {
            val cleanText = trimmed.replace(Regex("<[^>]*>"), "")
                .replace(Regex("^```[a-zA-Z0-9]*\\s*", RegexOption.MULTILINE), "")
                .replace(Regex("```\\s*$"), "")
                .replace("&quot;", "\"")
                .replace("&amp;", "&")
                .replace("&#39;", "'")
                ...
                .replace("&nbsp;", " ")
                .trim()
            return ModelDecision("Reply", cleanText, "", false)
        }
        ...
        try {
            ...
            var finalReply = if (reply.isEmpty() && type.equals("Reply", ignoreCase = true)) {
            ...
            finalReply = finalReply.replace(Regex("<[^>]*>"), "")
                .replace("&quot;", "\"")
                .replace("&amp;", "&")
                .replace("&#39;", "'")
                .replace("&apos;", "'")
    ...
    private fun fetchMemories() {
        val currentUser = auth.currentUser
        if (currentUser == null) {
            Log.d("ConvAgent", "No authenticated user. Loading memories from LocalMemoryStore.")
            cachedMemories = LocalMemoryStore.getMemories(this)
            Log.d("ConvAgent", "Loaded ${cachedMemories.size} memories from LocalMemoryStore")
            return
        }
        ...
        db.collection("users").document(currentUser.uid)
            .addSnapshotListener { snapshot, e ->
                if (e != null) {
                    Log.w("ConvAgent", "Listen failed. Falling back to LocalMemoryStore.", e)
                    cachedMemories = LocalMemoryStore.getMemories(this)
                ...
                if (snapshot != null && snapshot.exists()) {
                    ...
                    if (memoriesList != null) {
                        ...
                        Log.d("ConvAgent", "No memories field in user doc. Loading local.")
                        cachedMemories = LocalMemoryStore.getMemories(this)
                    }
                } else {
                    Log.d("ConvAgent", "Current data null. Loading local.")
                    cachedMemories = LocalMemoryStore.getMemories(this)
                }
            }
    }
