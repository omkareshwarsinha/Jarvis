    ...
    private suspend fun speak(text: String) {
        try {
            ...
            val cleanText = text.replace(Regex("<[^>]*>"), "")
                .replace(Regex("\\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Z|a-z]{2,}\\b"), "") // clean raw emails if any
            
            // Set language carefully (Hindi vs English/Hinglish)
            val isHindi = cleanText.any { it.code in 0x0900..0x097F } || 
                run {
                    val lc = cleanText.lowercase()
                    val keywords = listOf("hai", "karo", "kholo", "bajao", "bajaao", "gaana", "samay", "bhejo", "kar raha", "kar rahi", "shukriya", "batao", "bataiye", "chalao", "naam", "kaise", "hua", "raha", "namaste", "dhanyawad", "meri", "mera", "hu", "hoon")
                    keywords.any { kk -> lc.contains(kk) }
                }
            
            withContext(Dispatchers.Main) {
                if (isHindi) {
                    nativeTts?.language = java.util.Locale("hi", "IN")
                } else {
                    nativeTts?.language = java.util.Locale.US
                }
                
                // Show captions overlay
                OverlayDispatcher.show(cleanText, OverlayPriority.CAPTION, 4000L)
            }

            // Create a deferred to hold until speaking finishes
            val nativeTtsDeferred = CompletableDeferred<Unit>()
            nativeTts?.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
            ...
            nativeTts?.speak(cleanText, TextToSpeech.QUEUE_FLUSH, null, "NativeTTS_${System.currentTimeMillis()}")
            
            // Wait for speaking to complete or timeout to avoid blocking forever
            withTimeoutOrNull(20000) {
                nativeTtsDeferred.await()
            }
        } catch (e: Exception) {
            Log.e("TTSManager", "Native TTS speech failed", e)
        }
    }
