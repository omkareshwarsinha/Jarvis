...
object GoogleTts {
    ...
    suspend fun synthesize(text: String, voice: TTSVoice): ByteArray = withContext(Dispatchers.IO) {
        ...
        // Determine language code and voice based on text content
        ...
        val finalLanguageCode = if (isHindi) "hi-IN" else "en-US"
        val finalVoiceName = if (isHindi) "hi-IN-Neural2-C" else voice.voiceName

        // 1. Construct the JSON payload
        val jsonPayload = JSONObject().apply {
            put("input", JSONObject().put("text", text))
            put("voice", JSONObject().apply {
                put("languageCode", finalLanguageCode)
                put("name", finalVoiceName)
            })
