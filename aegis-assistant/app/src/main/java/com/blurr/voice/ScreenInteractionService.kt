...
//                // 1. Parse the raw XML into a structured list.
    ...
    private fun findInteractableNodesRecursive(
        ...
        try {
            ...
            // Continue searching through the children safely
            val count = node.childCount
            for (i in 0 until count) {
                try {
                    val child = node.getChild(i)
                    if (child != null) {
                        findInteractableNodesRecursive(child, list)
                    }
                } catch (e: Exception) {
                    // Ignore child fetching issues when a UI layout updates mid-traversal
                }
            }
        } catch (e: Exception) {
            Log.e("InteractionService", "Error during recursive node collection", e)
        }
    }
